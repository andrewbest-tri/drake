...
const DirectionUsageRule kDirectionUsage(DirectionUsageRule::Id("dur_id"),
                                         kZone,
                                         DirectionUsageRule::Type::kWithS,
...
GTEST_TEST(SimpleRulebookTest, AddGetRemoveDirectionUsageRule) {
  SimpleRulebook dut;
...
GTEST_TEST(SimpleRulebookTest, FindRules) {
  ...
  EXPECT_EQ(reversed.direction_usage.size(), 0);
}
